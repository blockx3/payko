...
type Chain = 'SOLANA';
type PaymentType = 'ANY' | 'NATIVECOIN' | 'TOKEN';
type AmountType = 'FLEXIBLE' | 'FIXED';
...
export default function PaymentPage({ paymentDetails, supportedTokens }: { paymentDetails: IPaymentDetails, supportedTokens: Token[] }) {
    const router = useRouter();
    const [selectedToken, setSelectedToken] = useState<Token | null>(null);
    const [amount, setAmount] = useState<string>(paymentDetails.amount?.toString() || '');
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 flex flex-col items-center justify-center p-4">
            <div className="w-full max-w-md bg-white rounded-xl shadow-xl overflow-hidden border border-gray-100 transition-all duration-300 hover:shadow-2xl">
                ...
                <div className="p-8">
                    {/* Title */}
                    <h1 className="text-3xl font-bold text-gray-800 mb-3">{paymentDetails.title}</h1>

                    {/* Description */}
                    {paymentDetails.description && (
                        <p className="text-gray-600 mb-8 leading-relaxed">{paymentDetails.description}</p>
                    )}
                        ...
                        <div className="relative rounded-lg shadow-sm">
                            ...
                            {selectedToken && (
                                <div className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                                    <span className="text-gray-500 font-medium">{selectedToken.symbol}</span>
                                </div>
                            )}
                        </div>
            ...
            {selectedToken && (
                <div className="mt-6 w-full max-w-md p-6 bg-white rounded-xl shadow-lg border border-gray-100 transition-all duration-300 hover:shadow-xl">
                    <div className="flex items-center">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold text-gray-800">{selectedToken.name} ({selectedToken.symbol})</h3>
                            <p className="text-sm text-gray-600">Token: <span className="font-mono text-xs bg-gray-100 px-2 py-1 rounded">{selectedToken.token_mint}</span></p>
                        </div>
                    </div>
                </div>
            )}
